...
Accordingly, any self-respecting organization would be well advised to invest in REST API security to ensure
confidentiality, integrity, and availability (i.e., the three fundamental tenets or objectives of information security,
colloquially known as the CIA-triad) of its data.
...
====
// TODO: if we agree on this proposal, we should provide details and examples
For it is strongly recommended that the category be clearly indicated on the media, a RESTful API SHOULD expose a
"/dataClassification" endpoint that returns the category of data being processed in accordance with the data
classification model provided by the FISP guide.
====
...
[.rule, caption="Rule {counter:rule-number}: "]
====
It's strongly RECOMMENDED not to enable TLS 1.0 in an attempt to provide support to legacy clients.
====
...
[.rule, caption="Rule {counter:rule-number}: "]
====
When data is to be persisted on disk, encryption at rest MUST be considered.
====
...
[.rule, caption="Rule {counter:rule-number}: "]
====
Tokens MUST only be stored either in memory or on cryptographic hardware through a secure storage API.
====
...
Self-contained tokens contain a set of claims stored in the header part of an
...
[.rule, caption="Rule {counter:rule-number}: "]
====
A resource server SHOULD validate the reserved claims provided in an access token’s header.
====
...
As such, they MUST NOT hold sensitive information!
====
...
https://docs.oasis-open.org/xacml/3.0/xacml-3.0-core-spec-os-en.html[XAML],
...
[.rule, caption="Rule {counter:rule-number}: "]
====
The key identifier MUST be provided in the "kid" (key identifier) claim.
====
...
[.rule, caption="Rule {counter:rule-number}: "]
====
All requests to RESTful services handling Category 1-2 data SHOULD be digitally signed by the API consumer using JWS.
====

[.rule, caption="Rule {counter:rule-number}: "]
====
All responses from RESTful services handling Category 1-2 data MUST be digitally signed by the API provider using JWS.
====
...
[.rule, caption="Rule {counter:rule-number}: "]
====
Although audit technical logs are typically collected for different purposes and kept separately, the two SHOULD have a
consistent format to be easily correlated.
====
